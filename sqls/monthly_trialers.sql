...
select
    ...
    , status
    , case
        when trial_end_at < cancel_at or cancel_at is NULL then 'Converted'
        else 'Not Converted'
    end as trial_status
...
select
  ...
  join generate_series(min_created_at, max_cancel_at + interval '1 month', '1 month'::interval) month on true
    ...
    select
        ...
        , coalesce(count(distinct case when trial_status = 'Not Converted' then subscription_id end), 0) as num_not_converted_post_trial
        , coalesce(count(distinct case when trial_status = 'Converted' then subscription_id end), 0) as num_converted_post_trial
    from trial_information
